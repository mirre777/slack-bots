...
const { getTrackedPackages } = require("./tools/tracking");
...
const tools = [
  ...
  {
    name: "ask_bot2",
    description: "Ask Bot 2 (research assistant) a question. Use for research, lookups, or when you need a second opinion. Bot 2 has no tools — it uses general knowledge only.",
    input_schema: {
      type: "object",
      properties: {
        question: { type: "string", description: "The question or task for Bot 2" },
      },
      required: ["question"],
    },
  },
  {
    name: "list_packages",
    description: "List all currently tracked packages with their status and tracking links.",
...
const toolHandlers = {
  ...
  ask_bot2: async (input) => {
    const response = await client.messages.create({
      model: "claude-sonnet-4-6",
      max_tokens: 1024,
      system: "You are a research assistant. Be concise, factual, and helpful. Respond in Dutch unless the question is in English.",
      messages: [{ role: "user", content: input.question }],
    });
    const text = response.content.find((b) => b.type === "text");
    return text ? text.text : "No response from Bot 2.";
  },
  list_packages: async () => {
    const pkgs = await getTrackedPackages();
    if (!pkgs.length) return "Geen pakketten worden gevolgd.";
    return pkgs.map((p) => `${p.description} — ${p.carrier}: ${p.trackUrl} (status: ${p.status})`).join("\n");
  },
...
async function askClaude(systemPrompt, messages, useTools = false) {
  // Support both string (legacy) and array (conversation history)
  const messageList = typeof messages === "string"
